...
namespace IOPlugin
{
    ...
    bool IPropertyProvider::GetUINT64(PropertyID p_ID, uint64_t& p_Val)
    {
        PropertyType type = propTypeNull;
        const void* pVal = NULL;
        int numVals = 0;
        const StatusCode err = GetProperty(p_ID, &type, &pVal, &numVals);
        if ((err != errNone) || (type != propTypeUInt64) || (numVals != 1))
        {
            return false;
        }

        p_Val = *static_cast<const uint64_t*>(pVal);
        return true;
    }
